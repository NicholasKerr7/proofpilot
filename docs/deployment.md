...
Password recovery stores only SHA-256 token hashes, enforces one-time redemption and expiry, invalidates older links when a new one is issued, and returns the same acknowledgement for known and unknown addresses. Development logs reset links with `PASSWORD_RESET_DELIVERY_MODE=log`; production startup requires `PASSWORD_RESET_DELIVERY_MODE=resend`, `RESEND_API_KEY`, and a verified `AUTH_EMAIL_FROM` sender.

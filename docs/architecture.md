...
Portfolio workspaces clone immutable sample evidence entities, processing logs, checklist matches, timeline source links, statement versions, submission rounds, submission updates, and packet export metadata with remapped IDs. Bundled packet PDFs and immutable evidence use `demo-samples/` storage markers and are never treated as visitor-owned objects. Reset expires and detaches only the current visitor's isolated graph, immediately provisions a fresh copy from the unchanged seed template, and lets the cleanup worker remove visitor-created storage before deleting the old records.
